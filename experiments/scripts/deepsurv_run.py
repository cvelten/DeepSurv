...
def parse_args():
    ...
    parser.add_argument(
        '--results_dir', help="Output directory to save results (model weights, visualizations)", default=None)
...
def save_risk_surface_visualizations(model, dataset, norm_vals, output_dir, plot_error, experiment,
    ...
    def risk_fxn(x): return np.squeeze(model.predict_risk(x))
...
def save_treatment_rec_visualizations(model, dataset, output_dir,
    ...
    rec_dict['rec_trt'] = model.recommend_treatment(dataset['x'], trt_i, trt_j, trt_idx)
